...
def diabet_var_2(number: int = None):
    ...
    print('Номер класса на основе нейронной сети (0-болен, 1-здоров): ', np.round(pred).astype(int))
    print('Номер класса в исходных данных (0-болен, 1-здоров): ', y_data[x])

    # Проверям правильность предсказания для 50-ти пациентов
    pred = model.predict(x_test_scaled)
    pred = np.round(pred).astype(int)
    print(pred[:30].reshape(-1), ' Результаты классификации нейросетью')
    print(y_test[:30].reshape(-1), ' Исходные данные')
    return score
